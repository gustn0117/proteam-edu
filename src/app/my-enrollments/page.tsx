              ...
              {enrollments.map((e) => {
                ...
                return (
                  <div key={e.id} className={`bg-white rounded-2xl border border-gray-100 overflow-hidden border-l-4 ${
                    e.enrollment_status === "completed" ? "border-l-blue-400" :
                    ...
                    <div className="p-5">
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-gray-50">
                        <div>
                          {e.enrollment_status === "completed" ? (
                            <Link href={`/certificate/${e.id}`} className="inline-flex items-center gap-1.5 bg-primary/5 text-primary hover:bg-primary/10 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors">
                    ...
                    {enrollments.map((e) => {
                      ...
                      return (
                        <tr key={e.id} className={`hover:bg-slate-50/50 transition-colors border-l-3 ${
                          ...
                          <td className="px-4 py-4 text-center">
                            {e.enrollment_status === "completed" ? (
                              <Link href={`/certificate/${e.id}`} className="inline-flex items-center gap-1.5 bg-gold/10 text-gold hover:bg-gold/20 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all hover:shadow-sm">
                                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" /></svg>
                                수료증 보기
                              </Link>
                            ) : (
